...
public class AgentInitDataListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent evt) {
		...
		try {
			...
			// String serverPath = grinderProperties.substring(0,
			// grinderProperties.indexOf("webapps"))+File.separator+"conf";
			String webRootDir = Thread.currentThread().getContextClassLoader().getResource("").getPath()
