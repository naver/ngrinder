<template>
    <div class="container perftest-list-container p-0">
        ...
        <vuetable
            ...
            :per-page="table.pagination.per_page"
            :css="table.css.table"
            :multi-sort="true"
            :fields="tableFields"
            :load-on-start="false"
            ...
            <template slot="status" slot-scope="props">
                <a class="ball"
                   ...
                   :data-content="`${props.rowData.progressMessage}<br><b>${props.rowData.lastProgressMessage}</b>`.replace(/\n/g, '<br>')">
                    <img :src="`/img/ball/${props.rowData.iconName}`">
                </a>
            </template>
            ...
            <template slot="startTime" slot-scope="props">
                <span v-if="props.rowData.startTime">{{ props.rowData.startTime | dateFormat('YYYY-MM-DD HH:mm') }}</span>
            </template>

            <template slot="tps" slot-scope="props">
                <span v-if="$utils.exists(props.rowData.tps)" v-text="props.rowData.tps.toFixed(1)"></span>
            </template>

            <template slot="meanTestTime" slot-scope="props">
                <span v-if="$utils.exists(props.rowData.meanTestTime)" v-text="props.rowData.meanTestTime.toFixed(1)"></span>
            </template>
            ...
            <template slot="actions" slot-scope="props">
                <i v-if="props.rowData.reportable" @click="showChart(props.rowData.id)"
                   :title="i18n('perfTest.action.showChart')" class="fa fa-line-chart pointer-cursor"></i>
                <i v-if="props.rowData.deletable" @click="deleteTests(props.rowData.id)"
                   :title="i18n('common.button.delete')" class="fa fa-remove pointer-cursor"></i>
                <i v-if="props.rowData.stoppable" @click="stopTest(props.rowData.id)"
...
<script>
    ...
    export default class PerfTestList extends Mixins(Base, MessagesMixin, TableConfig) {
        ...
        initByQueryParams() {
            ...
            this.table.pagination.per_page = parseInt(this.$route.query['page.size']) || this.table.pagination.per_page;
            this.sort = this.$route.query.sort || this.sort;
        }

        changePage(nextPage) {
            this.$refs.vuetable.changePage(nextPage);
            history.replaceState('', '', this.makeQueryString(this.$refs.vuetable.currentPage, this.table.pagination.per_page,
        ...
        beforePagination(paginationData) {
            ...
            paginationData.last_page = Math.ceil(paginationData.total / this.table.pagination.per_page);
            paginationData.per_page = this.table.pagination.per_page;
        ...
        beforeTableLoading() {
            ...
            this.table.appendParams['page.size'] = this.table.pagination.per_page;
        ...
        updateTableWithUrl() {
            history.replaceState('', '', this.makeQueryString(1, this.table.pagination.per_page, this.$refs.searchBar.searchText,
        ...
        runQueryFilter(queryFilter) {
            ...
            history.replaceState('', '', this.makeQueryString(0, this.table.pagination.per_page, this.$refs.searchBar.searchText,
                this.makeQueryFilter(), this.sort, this.$refs.searchBar.selectedTag));
            this.$refs.vuetable.refresh();
        }
