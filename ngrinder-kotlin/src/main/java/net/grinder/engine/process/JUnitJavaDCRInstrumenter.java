...
public class JUnitJavaDCRInstrumenter extends AbstractDCRInstrumenter {

	public JUnitJavaDCRInstrumenter(DCRContext context) {
		super(context);
	}
	...
	private void instrumentClass(Class<?> targetClass, Recorder recorder,
		...
		for (Method method : targetClass.getDeclaredMethods()) {
			if (Modifier.isStatic(method.getModifiers()) && filter.matches(method)) {
	...
	private void instrumentInstance(Object target, Recorder recorder,
		...
		do {
			for (Method method : c.getDeclaredMethods()) {
				if (!Modifier.isStatic(method.getModifiers()) && filter.matches(method)) {
					getContext().add(target, method, TargetSource.FIRST_PARAMETER, recorder);
				}
			}
