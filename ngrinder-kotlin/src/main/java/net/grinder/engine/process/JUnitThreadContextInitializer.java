...
public class JUnitThreadContextInitializer {
	...
	public void initialize() {
		...
		List<Instrumenter> list = new ArrayList<>();
		...
		list.add(new JUnitJavaDCRInstrumenter(context));
		...
		final Statistics scriptStatistics = new ScriptStatisticsImplementation(m_threadContexts, m_statisticsServices, new NullSender());

		final InternalScriptContext scriptContext = new ScriptContextImplementation(new SimpleWorkerIdentity("unit-test", 0),
			new SimpleWorkerIdentity("unit-test", 0), m_threadContexts, null,
			externalLogger, m_sleeper, null, scriptStatistics,
